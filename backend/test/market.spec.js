...
/// Later set tests for correct addresses and stuff
  describe("Functionality", function() {
    it("Should make a prediction market", async function () {
      ...
      /// 100 USDC base price
      /// make a market
      const makeMarket = tradingContract.connect(owner).createPrediction("ETH", mockDapi.address, true, "1900000000000000000000", 50, 1688702119, "100000000", owner.address)
      await makeMarket;
  ...
  describe("Load MarketHandler", function() {
    it("Should load the market handler correctly", async function () {
      ...
      /// Load marketHandler
      const marketHandler = new ethers.Contract(marketHandlerAddress, MarketHandler.abi);
